...
static void handle_syscall(trapframe *tf) {
  ...
  // you will encounter problems in later experiments! char* data = "Hello
  // world!\n"; long data_ptr = (long)data; do_syscall(64,data_ptr,1,0,0,0,0,0);
...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval) {
  ...
  case CAUSE_STORE_PAGE_FAULT:
  {
    ...
    // the virtual address that causes the page fault. void* pa = alloc_page();
    ...
    // USER_STACK_TOP - PGSIZE
    // user_vm_map((pagetable_t)current->pagetable, stval, PGSIZE, (uint64)pa,
    // prot_to_type(PROT_WRITE | PROT_READ, 0)); panic( "You need to implement
    // the operations that actually handle the page fault in lab2_3.\n" );
    break;
  }
  default:
...
void rrsched() {
  ...
  // schedule next process to run. panic( "You need to further implement the
  // timer handling in lab3_3.\n" );
}
